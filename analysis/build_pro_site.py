...
def figure(name,title,caption,csv):
    ...
    figure_number += 1
    return f'<figure><div class="figure-header"><div><span class="figure-number">Figure {figure_number:02d}</span><h3>{title}</h3></div><a href="assets/{name}" target="_blank" rel="noopener">Open chart ↗</a></div><iframe src="assets/{name}" title="{e(title)}" loading="lazy"></iframe><figcaption>{caption} <a href="analysis/{csv}">Download CSV ↓</a></figcaption></figure>'
