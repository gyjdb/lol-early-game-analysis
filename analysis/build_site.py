...
site = site.replace('</body>', '''<script>
const chapterLinks = [...document.querySelectorAll('.nav-links a[href^="#"]')];
const chapters = chapterLinks.map(a => document.querySelector(a.getAttribute('href')));
function updateReadingPosition() {
  const range = document.documentElement.scrollHeight - innerHeight;
  document.querySelector('header').style.setProperty('--read-progress', (range > 0 ? scrollY / range * 100 : 0) + '%');
  let current = null;
  chapters.forEach((section, i) => { if (section && section.getBoundingClientRect().top <= 180) current = i; });
  chapterLinks.forEach((link, i) => { if (i === current) link.setAttribute('aria-current', 'true'); else link.removeAttribute('aria-current'); });
}
